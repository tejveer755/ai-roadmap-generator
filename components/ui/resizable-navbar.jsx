...
export const NavBody = ({ children, className, visible }) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full bg-neutral-950 px-4 py-2 lg:flex",
        className
      )}>
      {children}
    </motion.div>
...
export const NavbarButton = ({ href, as: Tag = "a", children, className, variant = "dark", ...props }) => {
  const baseStyles =
    "px-4 py-2 rounded-md text-sm font-bold relative cursor-pointer hover:-translate-y-0.5 transition duration-200 inline-block text-center";

  const variantStyles = {
    primary: "bg-neutral-900 text-white shadow-[...]",
